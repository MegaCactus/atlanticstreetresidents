...
const CATS = ["water","hvac","elev","win","fire","amen","garage","pm","you","other"];
...
if (item.kind === "ticket") {
pub.incidents.unshift({ id: item.id, d: item.d, e: "", m: item.m, c: "you", t: "R", s: item.s, x: item.x, w: item.w, g: item.cat === "water" ? "hotwater" : "", dur: "", src: "ticket", img: item.img, cat: item.cat });
} else {
pub.reports.unshift({ id: item.id, d: item.d, title: item.title, text: item.text, where: item.where, cat: item.cat, img: item.img, anon: item.anon, name: item.name, at: item.at });
}
...
<script>
var CATS={water:"Hot water & plumbing",hvac:"Heating & cooling",elev:"Elevators",win:"Windows & exterior",fire:"Fire & life safety",amen:"Amenities & systems",garage:"Garage & common areas",pm:"Preventive maintenance",you:"Resident tickets",other:"Other"};
...
'<p class="meta">'+kind+(who?" from "+esc(who):"")+' filed '+esc(it.at.slice(0,16).replace("T"," "))+(it.contact?' (contact: '+esc(it.contact)+')':'')+'</p>'+
